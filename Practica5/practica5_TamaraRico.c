...
void IR_SendPacket(uint8_t *data, uint8_t len)
{
    ...
    //checksum -= *data;
    checksum = (~checksum) + 1;
    IR_SendByte((uint8_t)checksum);
    IR_SendByte((uint8_t)(checksum>>8));
}
...
{
    // *data llega vacio, aqui se almacena la cadena 
    // Preambulo // verify
    // Header    // verify
    // Payload   // process
    // Checksum
    ...
    uint8_t checksum8 = 0, len_received = 0, checksum16_r1 = 0, checksum16_r2 = 0;
    uint8_t checksum16_l = 0, checksum16_h = 0;
    uint16_t checksum16 = 0;

    int datos = uartAvailableData(IR_RX_UART_PORT);
    ...
    while (datos--) 
    {
        ...
        {
            if(c == 0){
                if(uartGetchar(IR_RX_UART_PORT) != 0xC0){
                    uartPuts(PC_UART_PORT, "\nPreambulo Error : Different from 0xC0");
                    return 0;
                }
            
                if(uartGetchar(IR_RX_UART_PORT) != 0xD1){
                    uartPuts(PC_UART_PORT, "\nPreambulo Error : Different from 0xD1");
                    return 0;
                }

                if(uartGetchar(IR_RX_UART_PORT) != 0xCE){
                    uartPuts(PC_UART_PORT, "\nPreambulo Error : Different from 0xCE");
                    return 0;
                }
            }
            c++;
            if(c == 1){
                len_received = uartGetchar(IR_RX_UART_PORT);
                checksum8 = (~len_received) + 1;
                if(uartGetchar(IR_RX_UART_PORT) != checksum8){
                    uartPuts(PC_UART_PORT, "\nCheckSum8 Error : Different data length");
                    return 0;
                }
            }
            ...
            *data = uartGetchar(IR_RX_UART_PORT);
            ...
            data++;
            if(len == len_received){
                checksum16_l = (~checksum16) + 1;
                if(uartGetchar(IR_RX_UART_PORT) != checksum16_l){
                    uartPuts(PC_UART_PORT, "\nCheckSum16_L Error : Different data send");
                    return 0;
                } 
                checksum16_h = ((~checksum16) + 1) >> 8;
                 if(uartGetchar(IR_RX_UART_PORT) != checksum16_h){
                    uartPuts(PC_UART_PORT, "\nCheckSum16_H Error : Different data send");
                    return 0;
                } 
            } 
...
void app_main(void)
{
    ...
    uint8_t capturedData[MAX_PACKET_SIZE]; //255
...
#define ENTER_KEY       13
    ...
    if (receivedLength)
    {
        //printf("\nReceived length = %X", receivedLength);
        ...
        if (uartKbhit(PC_UART_PORT))
        {
            ...
            switch (key)
            {
                ...
                case ENTER_KEY:
                {
                    if (capturedLength > 0)
                    {
                        //cuando se presiona enter se envia el paquete si y solo si se ha capturado algo 
